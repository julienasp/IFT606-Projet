...
class wp_vulnfinder_scan {
	...
	function vuln_admin_init() {
	  echo "<pre>";
	  $pdo = $this->vuln_pdo_create();
	  //vuln_pdo_reset($pdo);
	  
	  $vuln_table_exists = $this->vuln_pdo_table_exists($pdo, 'vuln_status');
	  if (!$vuln_table_exists) {
		$this->vuln_pdo_create_tables($pdo);
	  }
	  $this->vuln_pdo_load_md5($pdo);
	  $this->vuln_cron_file_log();
	  $this->vuln_pdo_print_digest_status($pdo);
	  $this->vuln_pdo_print_file_event($pdo);
	  $this->vuln_pdo_print_file_log($pdo);
	  echo "</pre>";
	}
